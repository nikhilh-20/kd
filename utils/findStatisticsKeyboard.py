...
parser = argparse.ArgumentParser(description='Find mean and standard deviation for kd.py timing data')
parser.add_argument('-files', metavar=('dwell_time', 'flight_time'), type=str, nargs=2, required=True, help='The file which has the timing data')
...
def writeToFile(typeIs, arrMean=[], arrDeviation=[]):
    ...
    if (typeIs == 'dwell'):
        with open(file_path + '/../dwellFlightkeyboard/dwellMean.csv' , 'a') as f:
            writer = csv.writer(f)
            writer.writerow(arrMean)
        with open(file_path + '/../dwellFlightkeyboard/dwellDeviation.csv' , 'a') as f:
            writer = csv.writer(f)
            writer.writerow(arrDeviation)
    else:
        with open(file_path + '/../dwellFlightkeyboard/flightMean.csv' , 'a') as f:
            writer = csv.writer(f)
            writer.writerow(arrMean)
        with open(file_path + '/../dwellFlightkeyboard/flightDeviation.csv' , 'a') as f:
            writer = csv.writer(f)
            writer.writerow(arrDeviation)
...
def main():
    ...
    dwellTime = openFile(args.files[0])
    ...
    flightTime = openFile(args.files[1])
